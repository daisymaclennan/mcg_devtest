  ...
  return (
    <Layout>
      ...
      <HorizontalScrollCanvas canvasProps={ canvasProps }>
        ...
        <ImageBackground src="/img/mcg-landing.png" alt="man" scrollY={scrollY} />
...
export async function getStaticProps() {
  const res = await fetch('http://localhost:8888/wp-json/wp/v2/front-page?order=asc')
  const frontPages = await res.json()

  const careersRes = await fetch('http://localhost:8888/wp-json/wp/v2/job-listing?order=asc')
  const careers = await careersRes.json()

  const brandsRes = await fetch('http://localhost:8888/wp-json/wp/v2/brand?order=asc')
